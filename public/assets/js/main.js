let selectAll = document.querySelectorAll('select.editable')
selectAll.forEach(sel => {
    sel.value = sel.getAttribute('data-value')
})

if (document.querySelectorAll('.localnumber')){
    let allNumber = document.querySelectorAll('.localnumber')
    if (allNumber){
...
if (document.querySelectorAll('.localdate')){
    let allNumber = document.querySelectorAll('.localdate')
    if (allNumber){
        allNumber.forEach(el => {
            if (el.textContent){
                let dd = new Date(el.textContent)

                el.textContent = `${dd.getDate()}/${dd.getMonth()+1}/${dd.getFullYear()} ${dd.getHours()}:${dd.getMinutes()}`
