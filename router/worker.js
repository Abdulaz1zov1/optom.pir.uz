...
router.post('/save',superauth,async(req,res)=>{
    const {_id,name,phone,phone_res,lvl,lname,salary,current} = req.body
    await Worker.findByIdAndUpdate(_id,{name,phone,phone_res,lvl,lname,salary,current})
...
router.post('/',superauth, async (req, res) => {
    const {name,phone,phone_res,lvl,lname,salary,current} = req.body
    let newWorker = await new Worker({name,phone,phone_res,lvl,lname,salary,current})
